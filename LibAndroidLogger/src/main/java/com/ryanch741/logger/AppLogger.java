...
//            Log.e(TAG, "flush interrupt.");
    ...
    private synchronized void clearLog(DeleteLogCallback callback, String... files) {
...
//                    Log.e(TAG, "flush success(" + buffer.size() + ").");
    ...
    private synchronized static File getOutFile() {
        if (null == filePrefix) {
            filePrefix = null != context ? context.getPackageName() : "com.ryanch741.logger";
        }
    ...
    public static synchronized void clear(Context context, DeleteLogCallback callback) {
        init(context);
        appLogger.clearLog(callback);
    }

    public static synchronized void clear(Context context, int beforeDay) {
        init(context);
        File file = getOutFile();
        ...
        if (files.size() > 0) {
            appLogger.clearLog(new DeleteLogCallback() {
                @Override
    ...
    private static class ALArrayList extends ArrayList<String> {
        private String name;
